...
import com.chy.order.util.BeanHump;
import com.chy.order.util.DataTableParameter;
import com.chy.order.util.DataUtil;
import com.chy.order.util.InstanceUtil;
import com.chy.order.util.PropertiesUtil;
	...
	public Page<T> selectPage(Map<String, Object> params, Class<T> cls){
		...
		pm.entrySet().stream().forEach(e->{
			if(likeStr.contains(e.getKey())){
				wrapper.like(e.getKey(), e.getValue().toString());
			}else{
